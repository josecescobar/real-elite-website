 ...
 * Larger, more deliberate than the sticky bar.
 *
 * `href`/`label` default to the free-estimate form so the bare
 * `<EstimateCTACard />` an author drops into MDX keeps behaving exactly as
 * before. GuideTemplate overrides them per article: a roofing guide sends the
 * reader to the instant roof quote instead, since that answers the question
 * the article just raised in about a minute.
 */
type Props = {
  heading?: string;
  body?: string;
